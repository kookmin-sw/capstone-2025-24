    ...
    try:
        while not stop:
            ...
            if now >= next_ts and len(recs) < CFG["WORKER"]["MAX_RECORDERS"]:
                ...
                # 각 객체의 신뢰도 확인 후 NN% 이상이면 추가
                for i, cls_id in enumerate(res.boxes.cls):
                    conf = res.boxes.conf[i].item()  # 현재 객체의 신뢰도
                    name = res.names[int(cls_id)]   # 현재 객체의 클래스 이름

                     # "weapon"이 95% 이상인 경우만 추가
